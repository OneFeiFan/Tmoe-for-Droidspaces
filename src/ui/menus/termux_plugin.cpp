...
std::shared_ptr<IUIMenu> TermuxMenuPlugin::build_repos_menu() {
    ...
    menu->add_child(std::make_shared<LambdaAction>(
        ...
        [](MenuContext&) -> bool { toggle_repo("root"); return true; }));

    menu->add_child(std::make_shared<LambdaAction>(
        _("termux.repo_x11"), "2",
        [](MenuContext&) -> bool { toggle_repo("x11"); return true; }));

    menu->add_child(std::make_shared<LambdaAction>(
        _("termux.repo_game"), "3",
        [](MenuContext&) -> bool { toggle_repo("game"); return true; }));

    menu->add_child(std::make_shared<LambdaAction>(
        _("termux.repo_science"), "4",
        [](MenuContext&) -> bool { toggle_repo("science"); return true; }));

    menu->add_child(std::make_shared<LambdaAction>(
        _("termux.repo_unstable"), "5",
        [](MenuContext&) -> bool { toggle_repo("unstable"); return true; }));
