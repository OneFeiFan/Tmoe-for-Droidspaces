                ...
                [this](MenuContext &ctx) -> bool {
                    ...
                    if (in_sudo) {
                        ...
                        if (ctx.cfg.linux_distro == "debian")
                            CommandBuilder("sudo").add_arg("deluser").add_arg(chosen).add_arg("sudo").add_raw(
                                    "2>/dev/null").execute();
        ...
        menu->add_child(std::make_shared<LambdaAction>(
                ...
                [this](MenuContext &ctx) -> bool {
                    ...
                    if (Executor::has("lolcat") || Executor::shell("test -f /usr/games/lolcat").ok())
                        Executor::passthrough(
                                "neofetch | /usr/games/lolcat 2>/dev/null || neofetch | lolcat 2>/dev/null || neofetch");
                    else
...
// ═════════════════════════════════════════════════════════════════════
    ...
    std::shared_ptr<IUIMenu> BetaFeaturesMenuPlugin::build_uefi_menu() {
        ...
        menu->add_child(std::make_shared<LambdaAction>(
                ...
                [this](MenuContext &ctx) -> bool {
                    ...
                    std::string input = Executor::tui_select(
                            ctx.cfg.tui_bin + " --title \"" + _("beta.uefi_boot_item_title") + "\" --inputbox \"" +
                            _("beta.uefi_boot_num_prompt") + "\" 0 0");
                    if (!input.empty())
                        CommandBuilder("sudo").add_arg("efibootmgr").add_flag("-o").add_arg(input).add_raw(
                                "2>/dev/null").execute();
                    Logger::press_enter();
                    return true;
                }));
        ...
        menu->add_child(std::make_shared<LambdaAction>(
                ...
                [this](MenuContext &ctx) -> bool {
                    std::string order = Executor::tui_select(
                            ctx.cfg.tui_bin + " --title \"" + _("beta.uefi_boot_order_title") + "\" --inputbox \"" +
                            _("beta.uefi_boot_order_prompt") + "\" 0 0");
                    if (!order.empty())
                        CommandBuilder("sudo").add_arg("efibootmgr").add_flag("-o").add_arg(order).add_raw(
                                "2>/dev/null").execute();
                    Logger::press_enter();
                    return true;
                }));
        ...
        menu->add_action(_("beta.uefi_backup_efi"), "3", [this] {
            ...
            if (!efi_disk.empty())
                CommandBuilder("sudo").add_arg("dd").add_arg("if=" + efi_disk).add_arg(
                        "of=/tmp/efi_backup.img").add_arg("bs=4M").add_raw("2>/dev/null").execute();
        ...
        menu->add_action(_("beta.uefi_restore_efi"), "4", [this] {
            Executor::passthrough(
                    "sudo dd if=/tmp/efi_backup.img of=$(df -h | grep '/boot/efi' | awk '{print $1}' | head -n1) bs=4M 2>/dev/null");
                    ...
                    if (ctx.cfg.linux_distro == "arch") {
                        ...
                        if (!deb.empty()) {
                            ...
                            Executor::passthrough("curl -L -o /tmp/" + deb + " "
                                                                             "'https://download1.rstudio.org/electron/focal/amd64/" +
                                                  deb + "' 2>/dev/null");
        ...
        menu->add_child(std::make_shared<LambdaAction>(
                ...
                [this](MenuContext &ctx) -> bool {
                    ...
                    std::string dmenu = ctx.cfg.tui_bin +
                                        " --title \"" + _("beta.scrcpy_device_title") + "\" --menu \"" +
                                        _("beta.scrcpy_device_switch_prompt") + "\" 0 0 0 ";
                    ...
                    if (dpick == "0" || dpick.empty()) {
                        Logger::press_enter();
                        return true;
                    }
                    int didx = std::stoi(dpick) - 1;
                    if (didx >= 0 && didx < (int) devices.size())
                        CommandBuilder("scrcpy").add_flag("-s").add_arg(devices[didx]).add_raw(
                                "2>/dev/null &").execute();
                    Logger::press_enter();
                    return true;
                }));
