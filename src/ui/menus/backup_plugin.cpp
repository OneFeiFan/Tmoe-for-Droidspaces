...
namespace tmoe::ui::menus {
    ...
    std::shared_ptr<IUIMenu> BackupMenuPlugin::build() {
        ...
        menu->add_child(std::make_shared<LambdaAction>(
                ...
                [this](MenuContext &) -> bool {
                    ...
                    if (fs::exists(rootfs)) {
                        mgr_->backup_container(name, rootfs);
                    } else {
        ...
        // ── 2. 恢复容器 ──
        menu->add_action(_("backup.restore_container"), "restore_container",
                         [this] { mgr_->restore_container(); });
        ...
        menu->add_child(std::make_shared<LambdaAction>(
                ...
                [this](MenuContext &) -> bool {
                    ...
                    if (fs::exists(rootfs)) {
                        mgr_->show_garbage_stats(rootfs);
                        if (Logger::confirm(_("backup.confirm_cleanup"))) {
                            mgr_->clean_container_garbage(rootfs);
        ...
        menu->add_child(std::make_shared<LambdaAction>(
                ...
                [this](MenuContext &) -> bool {
                    ...
                    mgr_->backup_to_external_storage(name, (cfg.container_root / name).string());
                    Logger::press_enter();
                    return true;
                }));

        // ── 5. 列出备份 ──
        menu->add_action(_("backup.list_backups"), "list_backups", [this] {
            auto backups = mgr_->list_backups_detailed();
            if (backups.empty()) {
                Logger::info(_("backup.none_found"));
            } else {
                Logger::info(_("backup.file_list_header") + mgr_->config().backup_dir.string() + "):");
        ...
        // ── 6. 空间占用排行 ──
        menu->add_action(_("backup.space_ranking"), "space_ranking", [this] {
            auto &cfg = mgr_->config();
            std::string path_cmd = cfg.tui_bin +
                                   " --title " + _("backup.cleanup_path_title") +
                                   " --inputbox " + _("backup.cleanup_path_input") + " 0 0 \"" +
        ...
        // ── 7. Timeshift ──
        menu->add_action(_("backup.timeshift"), "timeshift", [this] {
            auto &cfg = mgr_->config();
            ...
            if (ts_choice == "1") {
                mgr_->run_timeshift_backup();
            } else if (ts_choice == "2") {
                mgr_->install_timeshift();
            }
        });
        ...
        menu->add_child(std::make_shared<LambdaAction>(
                ...
                [this](MenuContext &) -> bool {
                    ...
                    auto fmt = mgr_->tui_select_format();
                    ...
                    if (backup_home) {
                        std::string home_file = "termux-home-" + mgr_->generate_filename("home", "", "") + ext;
                        Logger::step(_("backup.backing_up_home"));
                        mgr_->run_tar_backup_with_progress(termux_prefix + "/home",
                                                           (out_dir / home_file).string(), fmt);
                    }
                    if (backup_usr) {
                        std::vector<std::string> excludes = {termux_prefix + "/home/tmoe-linux/containers"};
                        std::string usr_file = "termux-usr-" + mgr_->generate_filename("usr", "", "") + ext;
                        Logger::step(_("backup.backing_up_prefix"));
                        mgr_->run_tar_backup_with_progress(termux_prefix + "/usr",
        ...
        // ── 9. 备份目录大小 ──
        menu->add_action(_("backup.backup_dir_size"), "backup_dir_size", [this] {
            auto &cfg = mgr_->config();
