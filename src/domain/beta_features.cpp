...
void BetaFeaturesManager::run_beta_menu() {
    while (true) {
        ...
        if (ch == "1")      install_single("beta.flameshot", "flameshot");
            ...
            PackageManager::install({"flatpak", "gnome-software-plugin-flatpak",
                                     "plasma-discover-backend-flatpak", "snapd"}, family);
        }
...
void BetaFeaturesManager::run_store_menu() {
    ...
    else if (ch == "2") PackageManager::install({"flatpak", "gnome-software-plugin-flatpak",
