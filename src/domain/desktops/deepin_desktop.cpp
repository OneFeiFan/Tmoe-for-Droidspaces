...
void DeepinDesktop::post_install_config(const PostInstallContext& ctx) {
    ...
    if (ctx.is_debian) {
        if (cfg_.sub_distro != "deepin")
            Executor::passthrough("apt update 2>/dev/null; apt install -y software-properties-common gnupg 2>/dev/null; yes | add-apt-repository ppa:ubuntudde-dev/stable 2>/dev/null || true");
