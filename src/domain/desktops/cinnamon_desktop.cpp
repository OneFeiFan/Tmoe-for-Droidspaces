...
namespace tmoe::domain {
    ...
    void CinnamonDesktop::post_install_config(const PostInstallContext &ctx) {
        ...
        else if (ctx.family == DistroFamily::RedHat)
            PackageManager::install("@'Cinnamon Desktop'", DistroFamily::RedHat);
        else if (ctx.family == DistroFamily::Arch)
            PackageManager::install({"cinnamon-translations", "cinnamon"}, ctx.family);
