...
// ── 阶段2: mate/ubuntu-mate + core/lite + Linux Mint ──
    PreInstallChoices MateDesktop::pre_install_choices(
        ...
        if (is_ubuntu) {
            if (ui::dialog::yesno(cfg_, "Mate or Ubuntu-MATE", _("gui.desktop.mate_or_ubuntu_desc"), "mate",
                                  "ubuntu-mate") == 1) {
                c.pkg_list = "ubuntu-mate-desktop";
                return c;
            }
        }

        // core / lite
        if (ui::dialog::yesno(cfg_, "MATE-CORE or MATE-LITE", _("gui.desktop.mate_core_or_lite_desc"), "core",
                              "lite") != 0) {
