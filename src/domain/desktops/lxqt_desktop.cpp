...
namespace tmoe::domain {
    ...
    void LxqtDesktop::will_be_installed_message() const {
        ...
        Logger::info(_("gui.lxqt.package_list"));
        ...
        if (family == DistroFamily::Debian && Executor::has("apt-cache"))
            Executor::passthrough("apt-cache show lxqt-core 2>/dev/null | head -20 || true");
    }
    ...
    PreInstallChoices LxqtDesktop::pre_install_choices(
        ...
        if (is_ubuntu) {
            if (ui::dialog::yesno(cfg_, "Lxqt or Lubuntu-desktop", _("gui.desktop.lxqt_or_lubuntu_desc"), "lxqt",
                                  "lubuntu") == 1) {
        ...
        if (ui::dialog::yesno(cfg_, "LXQT-CORE or LXQT-LITE", _("gui.desktop.lxqt_core_or_lite_desc"), "core",
                              "lite") != 0) {
    ...
    void LxqtDesktop::post_install_config(const PostInstallContext &ctx) {
        ...
        if (!Logger::confirm_yes_default(_("gui.lxqt.confirm_install"))) return;
