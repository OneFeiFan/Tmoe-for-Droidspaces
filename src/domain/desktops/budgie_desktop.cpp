...
namespace tmoe::domain {
    ...
    PreInstallChoices BudgieDesktop::pre_install_choices(DistroFamily f, bool a) {
        ...
        if (a) return c;
        // 检查不支持的发行版
        if (f != DistroFamily::Debian && f != DistroFamily::Arch && f != DistroFamily::Void_ &&
            f != DistroFamily::Solus) {
        ...
        if (cfg_.sub_distro == "ubuntu") {
            if (ui::dialog::yesno(cfg_, "budgie/ubuntu-budgie", "budgie/ubuntu-budgie?", "budgie", "ubuntu-budgie") ==
                1) {
    ...
    void BudgieDesktop::post_install_config(const PostInstallContext &ctx) {
        ...
        SystemHelper::write_file("/usr/local/bin/budgie-desktop-builtin",
                                 "#!/bin/sh\nbudgie-wm --x11 --replace &\nbudgie-panel --replace &\nwait\n");
        CommandBuilder("sudo").add_arg("chmod").add_arg("a+rx").add_arg(
                "/usr/local/bin/budgie-desktop-builtin").execute();
