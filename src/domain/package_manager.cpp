...
bool PackageManager::install(const std::vector<std::string>& pkgs, DistroFamily family) {
    ...
    Logger::info(_("app.installing") + " " + install_line);

    if (Executor::shell(install_line).ok()) {
    ...
    if (cmd.use_eatmydata) {
        ...
        if (Executor::shell(fallback).ok()) return true;
    }
...
bool PackageManager::remove(const std::vector<std::string>& pkgs, DistroFamily family) {
    ...
    Logger::info(_("app.removing") + " " + remove_line);
    return Executor::shell(remove_line).ok();
}

bool PackageManager::update(DistroFamily family) {
    auto cmd = commands_for(family);
    if (cmd.update.empty()) return true; // 某些发行版不需要单独 update
    Logger::info(_("app.updating") + " " + cmd.update);
    return Executor::shell(cmd.update).ok();
}
...
bool PackageManager::ensure_eatmydata() {
    ...
    Logger::info(_("app.installing_eatmydata"));
    if (Executor::shell("apt install -y eatmydata").ok()) return true;
    return Executor::shell("apt install -y -f eatmydata").ok();
}
