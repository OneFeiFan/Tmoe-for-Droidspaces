...
public:
    ...
    /** QEMU User Static 管理子菜单。 */
    void tmoe_qemu_user_static();

    /** 跨架构运行 TUI 菜单。 */
    void run_docker_across_architectures(bool systemd_mode = false);
...
private:
    ...
    // ── 跨架构辅助 ──
    /** 设置跨架构环境变量。 */
    std::string setup_cross_arch_env(int arch_choice);
    std::string qemu_arch_;
    std::string new_arch_prefix_;
    std::string container_ext_name_;
