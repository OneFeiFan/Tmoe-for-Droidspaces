...
// 主菜单
// ═══════════════════════════════════════════════════════════
void MediaTools::run_image_compression_menu() {
    while (true) {
        std::string menu = cfg_.tui_bin
            + " --title \"" + _("media.compress.title") + "\""
            + " --menu \"" + _("media.compress.menu_prompt") + "\" 0 0 0 "
            "\"1\" \"" + _("media.compress.start") + "\" "
            "\"2\" \"" + _("media.compress.install_dep") + "\" "
            "\"3\" \"" + _("media.compress.remove") + "\" "
            "\"0\" \"" + _("menu.tui.back") + "\"";
        auto ch = Executor::tui_select(menu);
        if (ch == "0" || ch.empty()) return;

        if (ch == "1") start_compression();
        else if (ch == "2") install_dependencies();
        else if (ch == "3") remove_dependencies();
        Logger::press_enter();
    }
}
...
void MediaTools::install_dependencies() {
    ...
    std::string img_pkg;
    switch (family) {
    case DistroFamily::Debian: img_pkg = "graphicsmagick-imagemagick-compat"; break;
    case DistroFamily::RedHat: img_pkg = "ImageMagick"; break;
    default: img_pkg = "imagemagick"; break;
    }
