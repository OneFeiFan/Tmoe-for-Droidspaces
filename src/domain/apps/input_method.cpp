...
namespace tmoe::domain {
    ...
    void InputMethodManager::write_input_env_vars() {
        ...
        std::string env_block =
                ...
                "XMODIFIERS=@im=fcitx\n"
                "SDL_IM_MODULE=fcitx\n";
        Executor::shell(CommandBuilder("echo").add_arg(env_block).add_raw(
                "| sudo tee -a /etc/environment >/dev/null 2>&1 || true").build_string());
        Logger::ok(_("input.env_done"));
    }

    void InputMethodManager::setup_fcitx_autostart() {
        Logger::step(_("input.autostart"));
        Executor::shell(CommandBuilder("sudo").add_arg("mkdir").add_flag("-p").add_arg("/etc/xdg/autostart").add_raw(
                "2>/dev/null").build_string());
        Executor::shell(CommandBuilder("mkdir").add_flag("-p").add_arg("~/.config/autostart").add_raw(
                "2>/dev/null").build_string());
