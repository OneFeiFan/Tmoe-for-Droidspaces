...
// ═══════════════════════════════════════════════════════════════
    ...
    void BackupManager::show_garbage_stats(std::string_view rootfs_path) {
        ...
        auto result = Executor::shell(
                CommandBuilder("du").add_flag("-sh").add_arg(root).add_raw("2>/dev/null").build_string());
        ...
        for (const auto &[label, path]: cache_dirs) {
            if (fs::exists(path)) {
                auto r = Executor::shell(CommandBuilder("du").add_flag("-sh").add_arg(path).add_raw(
                        "2>/dev/null | cut -f1").build_string());
...
// ═══════════════════════════════════════════════════════════════
    ...
    BackupManager::RestoreMode BackupManager::tui_select_restore_mode() {
        ...
        using namespace tmoe::ui;
        auto menu = make_plugin_menu(_("backup.restore_mode_title"), _("backup.restore_mode_prompt"),
                                     "backup_restore_mode");
