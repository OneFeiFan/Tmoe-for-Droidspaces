...
// ═══════════════════════════════════════════════════════════════════
    ...
    bool ConfigManager::apply_dns(const std::string &provider_id) {
        ...
        if (fs::exists("/etc/resolv.conf")) {
            Executor::shell(CommandBuilder("sudo").add_arg("cp").add_flag("-f").add_arg("/etc/resolv.conf").add_arg(
                    "/etc/resolv.conf.tmoe.bak").add_raw("2>/dev/null").build_string());
        }
...
// ═══════════════════════════════════════════════════════════════════
    ...
    bool ConfigManager::apply_timezone(const std::string &tz) {
        ...
        if (Executor::has("timedatectl")) {
            ok = Executor::shell(CommandBuilder("sudo").add_arg("timedatectl").add_arg("set-timezone").add_arg(
                    tz).build_string()).ok();
        }
        if (!ok) {
            ok = Executor::shell(
                    CommandBuilder("sudo").add_arg("ln").add_flag("-sf").add_arg("/usr/share/zoneinfo/" + tz).add_arg(
                            "/etc/localtime").add_raw("2>/dev/null").build_string()).ok();
            if (ok) {
    ...
    std::string ConfigManager::detect_current_timezone() const {
        ...
        auto r = Executor::shell(
                CommandBuilder("timedatectl").add_arg("show").add_arg("--property=Timezone").add_arg("--value").add_raw(
        ...
        r = Executor::shell(CommandBuilder("readlink").add_flag("-f").add_arg("/etc/localtime").add_raw(
                "2>/dev/null").build_string());
        if (r.ok() && !r.stdout_data.empty()) {
        ...
        // 方法4: Android getprop
        r = Executor::shell(
                CommandBuilder("getprop").add_arg("persist.sys.timezone").add_raw("2>/dev/null").build_string());
        if (r.ok() && !r.stdout_data.empty()) {
            std::string tz = r.stdout_data;
            tz.erase(std::remove(tz.begin(), tz.end(), '\n'), tz.end());
            if (!tz.empty()) return tz;
...
// ═══════════════════════════════════════════════════════════════════
    ...
    bool ConfigManager::change_root_password() {
        ...
        auto result = Executor::shell(
                CommandBuilder("echo").add_arg(chpasswd_input).add_raw("| sudo chpasswd 2>/dev/null").build_string());
        ...
        if (result.ok()) {
            ...
            result = Executor::shell(CommandBuilder("echo").add_arg(pw1).add_raw(
                    "| sudo passwd --stdin root 2>/dev/null").build_string());
            if (result.ok()) {
                Logger::ok(_("passwd.changed"));
                return true;
            }
...
// ═══════════════════════════════════════════════════════════════════
    ...
    bool ConfigManager::configure_hostname() {
        ...
        if (Executor::has("hostnamectl")) {
            ok = Executor::shell(CommandBuilder("sudo").add_arg("hostnamectl").add_arg("set-hostname").add_arg(
                    new_hostname).build_string()).ok();
        } else {
            ok = write_config_file("/etc/hostname", new_hostname + "\n");
            if (ok) {
                Executor::shell(CommandBuilder("sudo").add_arg("hostname").add_arg(new_hostname).add_raw(
