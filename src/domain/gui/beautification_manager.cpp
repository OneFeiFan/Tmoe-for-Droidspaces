        ...
        if (!fs::exists(colorscheme_dir + "/Monokai Remastered.theme")) {
            ...
            if (fs::exists(tmp_arc)) {
                ...
                Executor::passthrough(
                        "cd '" + colorscheme_dir + "' && sudo tar -Jxf '" + tmp_arc + "' 2>/dev/null || true");
                std::error_code ec;
...
${color #4080ff}Net: ${color white}${addr wlan0} ${addr eth0}
    ...
    void BeautificationManager::download_win10x_theme() {
        ...
        Executor::shell(
                "sudo update-icon-caches /usr/share/icons/We10X-Valley-dark /usr/share/icons/We10X-Valley 2>/dev/null &");
    ...
    void BeautificationManager::download_candy_icon_theme() {
        ...
        // 双源下载
        if (!SystemHelper::download_file("https://github.com/EliverLara/candy-icons/archive/refs/heads/master.zip",
                                         tmp_zip))
            SystemHelper::download_file(
                    "https://ghproxy.com/https://github.com/EliverLara/candy-icons/archive/refs/heads/master.zip",
                    tmp_zip);
    ...
    void BeautificationManager::download_paper_icon_theme() {
        ...
        if (SystemHelper::fetch_latest_and_extract("https://mirrors.bfsu.edu.cn/manjaro/pool/overlay/",
            ...
            Executor::shell(
                    "sudo update-icon-caches /usr/share/icons/Paper /usr/share/icons/Paper-Mono-Dark 2>/dev/null &");
        }
    ...
    void BeautificationManager::download_manjaro_wallpaper() {
        ...
        SystemHelper::download_file(
                "https://mirrors.tuna.tsinghua.edu.cn/manjaro/pool/overlay/wallpapers-2018-1.2-2-any.pkg.tar.zst",
                tmp1);
        if (fs::exists(tmp1)) {
            SystemHelper::extract_archive(tmp1);
            std::error_code ec;
            fs::remove(tmp1, ec);
        }
        // 找不到下载地址，暂时注释
    ...
    void BeautificationManager::download_arch_xfce_artwork() {
        ...
        SystemHelper::fetch_latest_and_extract(
                "https://mirrors.tuna.tsinghua.edu.cn/ubuntu/pool/universe/x/xfce4-artwork/",
    ...
    void BeautificationManager::download_ubuntu_kylin_wallpaper() {
        ...
        SystemHelper::fetch_latest_and_extract(
                "https://mirrors.bfsu.edu.cn/ubuntu/pool/universe/u/ubuntukylin-wallpapers/",
