...
namespace tmoe {
    ...
    bool SystemHelper::download_file(const std::string &url, const std::string &dest_path) {
        auto curl_cmd = CommandBuilder("curl").add_flag("-#fL").add_arg(url).add_arg("-o").add_arg(dest_path).
                build_string();
        auto wget_cmd = CommandBuilder("wget").add_flag("--show-progress").add_arg("-O").add_arg(dest_path).add_arg(url)
                .build_string();
        auto aria2_cmd = CommandBuilder("aria2c").add_flag("--no-conf").add_flag("--allow-overwrite=true").add_arg("-o")
                .add_arg(dest_path).add_arg(url).build_string();
